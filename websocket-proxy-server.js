...
const N8N_WEBHOOK_URL = 'https://ndrozd6261.app.n8n.cloud/webhook/uw-live-signal';

// Unusual Whales WebSocket endpoint (OFFICIAL)
const WS_URL = `wss://api.unusualwhales.com/socket?token=${UW_API_KEY}`;
...
function connectWebSocket() {
  ...
  try {
    // Create WebSocket connection (token in URL, no headers needed)
...
function handleOpen() {
  ...
  // Subscribe to channels (official channel names from UW docs)
  const subscribeMessage = {
    action: 'subscribe',
    channels: [
      'flow-alerts',      // Real-time flow alerts (high premium)
      'off_lit_trades'    // Dark pool prints
    ]
  };
...
function handleMessage(data) {
  ...
  try {
    ...
    // Route to appropriate handler (official UW channel names)
    if (message.type === 'flow-alerts' || message.channel === 'flow-alerts') {
      processOptionsFlow(message.data || message);
    } else if (message.type === 'off_lit_trades' || message.channel === 'off_lit_trades') {
      processDarkPool(message.data || message);
    } else if (message.type === 'pong' || message.event === 'pong') {
      // Heartbeat response
    } else if (message.type === 'subscribed' || message.event === 'subscribed') {
      console.log('✅ Subscribed to channel:', message.channel);
    } else {
      console.log('📨 Message:', JSON.stringify(message).substring(0, 200));
    }
